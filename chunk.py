    ...
    def do_GET(self):
        if self.path.startswith("/progress"):
            ...
            if '?' in self.path:
                ...
                order_no_param = params["order_no"][
                    0] if "order_no" in params else None
                if order_no_param == None:
                    ...
                    total, curr = r.get(order_no_param + "_total"), r.get(
                        order_no_param + "_curr")
        ...
        if self.path.startswith("/skus/check"):
            ...
            if '?' in self.path:
                ...
                batch_size_param = params["batch_size"][
                    0] if "batch_size" in params else None
                interval_param = params["interval"][
                    0] if "interval" in params else None
            try:
                batch_size = int(
                    batch_size_param
                ) if batch_size_param is not None else config.batch_size
            except Exception, e:
                print e.message
            ...
            try:
                interval = int(
                    interval_param
                ) if interval_param is not None else config.interval
            except Exception, e:
                print e.message
            ...
            try:
                ...
                if redis_total != None:
                    r.setex(order_no + "_total", total + int(redis_total),
                            86400)
                else:
                ...
                for attribute, value in json_obj[
                        config.chunk_field].iteritems():
                    curr += 1
                    ...
                    if size == batch_size:
                        ...
                        response_inner = requests.post(
                            "http://139.196.183.93:8088/wms/wsAction_doInventorySubmit.action",
                            data=json_obj_final)
